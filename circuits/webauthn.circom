...
template WebAuthnVerify(n, k, max_auth_data_bytes) {
  /// Private Inputs
  signal input r[k];
  signal input s[k];
  ...
  for (var i = 0; i < max_auth_data_bytes; i++) {
    ...
    for (var j = 0; j < 8; j++) {
      concatenator.first[i*8 + j] <== auth_data_bitify[i].out[j];
    }
  }
  ...
  for (var i = 0; i < k-1; i++) {
    numify[i] = Bits2Num(n);
    for (var j = 0; j < n; j++) {
      numify[i].in[j] <== message_hasher.out[i*n + j];
    }
...
component main = WebAuthnVerify(43,6, 64);